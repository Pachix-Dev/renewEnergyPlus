...
export function StepThreeSitio({ translates, currentLanguage }) {
  const {
    name,
    paternSurname,
    maternSurname,
  ...
  const handleRegister = async () => {
    ...
    const response = await fetch(urlbase + 'free-register-sitio', {
      method: 'POST',
      headers: {
    ...
    if (orderData?.status) {
      ...
      currentLanguage === 'es'
        ? (window.location.href = '/gracias-por-registrarte-en-sitio')
        : (window.location.href = '/en/gracias-por-registrarte-en-sitio')
    } else {
      setProcessing(false)
